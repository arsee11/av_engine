...
{
public:
    static AvFrameScaleFilter* create(PixelFormat f, int width, int height, Transformation<AVParam>* next_filter=nullptr){
		return new AvFrameScaleFilter(f, width, height, next_filter); }
    
private:
    AvFrameScaleFilter(PixelFormat f, int width, int height, Transformation<AVParam>* next_filter=nullptr)
